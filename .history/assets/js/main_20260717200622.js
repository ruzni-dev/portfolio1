...
if (cursorOutline && window.matchMedia('(pointer: fine)').matches) {
    ...
    const setCursorState = (target) => {
        const isInteractiveTarget = target && target.closest && target.closest(interactiveCursorTargets);

        cursorOutline.classList.toggle('is-active', Boolean(isInteractiveTarget));
