...
if (cursorOutline && homeSection && window.matchMedia('(pointer: fine)').matches) {
    let homeCursorActive = false;

    const setHomeCursorState = (insideHome) => {
        homeCursorActive = insideHome;
    ...
    const isPointerInsideHome = (x, y) => {
        const rect = homeSection.getBoundingClientRect();
        return x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom;
    };

    window.addEventListener('mousemove', (e) => {
        const insideHome = isPointerInsideHome(e.clientX, e.clientY);

        if (insideHome !== homeCursorActive) {
            setHomeCursorState(insideHome);
        }
